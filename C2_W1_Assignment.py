...
def get_probs(word_count_dict):
    ...
    for word in word_count_dict:
        probs[word] = word_count_dict[word]/len(word_l)
...
def edit_two_letters(word, allow_switches = True):
    ...
    #edit_two_set = [delete_letter(word) + switch_letter(word) + replace_letter(word) \
                #      + insert_letter(word) for word in edit_one_letter(word)]
    edit_two_set = set()
    ### START CODE HERE ###
    elemnts = set(edit_one_letter(word))
    #print(elemnts)
    for i in elemnts:
        edit_two_set = edit_two_set.union(edit_one_letter(i))
...
word = 'dys'
suggestions = set(edit_one_letter(word) & edit_two_letters(word))
#suggestions
...
def get_corrections(word, probs, vocab, n=2, verbose = False):
    ...
    n_best = [(word,probs[word]) for word in n_top]
